...
#experimental data
def dump_all_images( parent_path, arrays, total=None ):
    n_array = np.asarray(  255.0 * (arrays - np.amin( arrays )) / ( np.amax(arrays) - np.amin(arrays) +1.0e-10 ), dtype='uint8' )
    ...
    for idx in range( total ):
        ...
        imageio.imsave( file_name, n_array[idx] )
...
import os.path
from multidomain_wall_mirror_generator_16th import build_model
...
for iteration in range( iterations ):
    ...
    e_512_all, e_256_all, e_128_all, e_64_all, e_32_all = generator.predict( test_camera_captured_images[0:to_predict], batch_size=batch_size )
