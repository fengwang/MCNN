...
def train_mdcnn( model_path='./model/MDCNN-III-hela-I-1_1.h5', image_shape=(512, 512), epochs=1024, batch_size=8, gpus=0, index=0, input_layers=None, output_layers=None ):
    ...
    i_image_se = imageio.imread( '/data/experimental/Feng2018/HeLa_cell_focal_series/Pos0/img_000000000_Default0_026.tif' )
    ...
    for idx in range( epochs ):
        ...
        #mdcnn.save( f'./hela_defocus-1/_1__current_model_{index}_{idx}.model' )

        output_image, *_ = mdcnn.predict( input_image )
        output_image = output_image.reshape( (row, col) )
        imageio.imsave( f'./hela_defocus-1/_1__{index}_{idx}_prediction.png', output_image )

        output_image_se, *_ = mdcnn.predict( input_image_se )
        output_image_se = output_image_se.reshape( (row, col) )
        imageio.imsave( f'./hela_defocus-1/_1__{index}_{idx}_prediction_se.png', output_image_se )
...
if __name__ == '__main__':
    ...
    for idx in range( 10000 ):
        train_mdcnn( gpus=1, epochs=2, batch_size=4, model_path = './hela_defocus-1/current_model_-1.model', index=idx, input_layers=input_layers, output_layers=output_layers )
